...
- **Run all commands from the repo root** `/Users/colschulz/projects/personal/skein` unless stated otherwise.
...
```json
{
  ...
  "scripts": {
    "build:shared": "npm run build -w @skein/shared",
    "dev:server": "npm run dev -w @skein/server",
    "dev:web": "npm run dev -w @skein/web",
...
Run: `npm run test -w @skein/shared`
...
describe("eutils URL builders", () => {
  ...
  it("includes api_key when SKEIN_NCBI_API_KEY is set", () => {
    process.env.SKEIN_NCBI_API_KEY = "abc123";
    const u = new URL(esearchUrl("Smith J", 10));
    expect(u.searchParams.get("api_key")).toBe("abc123");
    delete process.env.SKEIN_NCBI_API_KEY;
  });
});
```
...
Run: `npm run test -w @skein/server`
...
Edit `apps/web/package.json` to set `"name": "@skein/web"` and add dependencies:
...
Run: `npm run test -w @skein/web && npx tsc -p apps/web/tsconfig.json --noEmit`
...
- **Rate limits:** without `SKEIN_NCBI_API_KEY`, NCBI allows ~3 req/s. The lru cache plus TanStack Query keep you well under this for normal exploration.
